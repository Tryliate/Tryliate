...
import { Home, Sparkles, Server, Search } from 'lucide-react';
import { Badge } from '../../ui/Badge';
import { Tooltip } from '../../ui/Tooltip';

const mainNav = [
  { id: 'dashboard', label: 'Dashboard', icon: Home },
  { id: 'studio', label: 'Build Workflow', icon: Sparkles, badge: 'v1.1.1' },
  { id: 'registry', label: 'Neural Engine', icon: Server, badge: 'Live' },
];
...
export const Navigation: React.FC<NavigationProps> = ({ activeView, onViewChange, onCollapse }) => {
  return (
    <div style={{ padding: '24px 24px 0 24px', flexShrink: 0 }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
          <Tooltip content="Close sidebar" shortcut="⌘\" align="right">
            <div
              onClick={onCollapse}
              style={{
                width: '32px',
      ...
      <div style={{ marginBottom: '16px', paddingLeft: '4px', fontSize: '12px' }}>
        Navigation
      </div>
        ...
        {mainNav.map((item) => {
          ...
          return (
            ...
            >
              ...
              <item.icon size={16} strokeWidth={2} style={{ marginRight: '10px' }} />
              <span style={{ flex: 1 }}>{item.label}</span>
              {item.badge && (
                <Badge variant={item.badge.toLowerCase() as any}>
                  {item.badge}
                </Badge>
              )}
            </div>
