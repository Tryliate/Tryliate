...
/**
 * Pre-configured OAuth templates
 * Users still need to provide their own client ID/secret
 */
export const INTEGRATION_PROVIDER_TEMPLATES: Record<string, IntegrationProviderTemplate> = {
  // Communication
  'slack': {
    auth_mode: 'OAUTH2',
 ...
 */
export function getProviderTemplate(providerKey: string): IntegrationProviderTemplate | null {
  return INTEGRATION_PROVIDER_TEMPLATES[providerKey.toLowerCase()] || null;
}
...
export function getAllProviderKeys(): string[] {
  return Object.keys(INTEGRATION_PROVIDER_TEMPLATES);
}
...
export function getProvidersByCategory(category: string): string[] {
  return Object.entries(INTEGRATION_PROVIDER_TEMPLATES)
